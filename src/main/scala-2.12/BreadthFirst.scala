...
object BreadthFirst {
  ...
  def tileSlide(puzzleTable: IndexedSeq[IndexedSeq[Int]]): Vector[Direction] = {
    ...
    def recur(queue: Queue[Node], trackSet: Set[IndexedSeq[IndexedSeq[Int]]]): Vector[Direction] = {
      ...
      else recur(queue.dequeue._2.enqueue(neighbors(currentNode, trackSet)), trackSet + currentNode.content)
    }
  ...
  def hanoi(numOfDisks: Int): Vector[(Int, Int)] = {
    ...
    recur(Queue(Node(startingContent, Vector.empty[(Int, Int)])), Set.empty[IndexedSeq[List[Int]]])
